...
namespace PlugHub.Plugin.DockHost.Interfaces.Plugins
{
    ...
    /// Optional Avalonia <see cref="Control"/> type to instantiate for this panel. 
    /// If provided, the host will resolve or construct it via DI.
    /// </param>
    /// <param name="ViewModelType">
    /// Optional view-model type. If provided alongside <paramref name="ContentType"/>, 
    /// the host will resolve it and assign it to the view's DataContext.
    /// </param>
    /// <param name="Factory">
    /// Optional factory function to create the panel's content control. 
    /// Used if <paramref name="ContentType"/> is not provided.
    /// </param>
    /// <param name="TargetedHosts">
    /// Optional dock host identifiers. If set, the panel is bound only to controls with matching IDs; 
    /// otherwise the panel is unbound and may appear on any host.
    /// </param>
