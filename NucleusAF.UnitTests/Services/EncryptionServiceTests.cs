...
namespace NucleusAF.UnitTests.Services
{
    ...
    public class EncryptionServiceTests
    {
        ...
        internal class UnitTestSecureAConfig
        {
            ...
            [Secure] public string FieldB { get; set; } = "Secrets!";

            public SecureValue FieldC { get; set; } = new SecureValue("More Secrets!");
        }
        internal class UnitTestSecureBConfig
        {
            [Secure] public required string FieldA { get; set; } = "NucleusAF";

            public int FieldB { get; set; } = 100;

            [Secure] public float FieldC { get; } = 3.14f;
        }
        ...
        public async Task Ctor_GeneratesPrimaryKey_WhenAbsent()
        {
            ...
            byte[]? persisted = (await this.storage!
                    .TryLoadAsync("NucleusAF-masterkey"))
                    ?.ToArray();
        ...
        [TestMethod]
        [TestCategory("EncryptDecrypt")]
        public void Encrypt_NullPlainText_Throw()
        {
            // Arrange
            byte[] key = new byte[32]; RandomNumberGenerator.Fill(key);
        ...
        public void Decrypt_WithWrongKey_Throw()
        {
            // Arrange
        ...
        [TestMethod]
        [TestCategory("Security")]
        public void Decrypt_WithInvalidMarker_Throw()
        {
            // Arrange
            byte[] key = new byte[32]; RandomNumberGenerator.Fill(key);
