...
namespace NucleusAF.Services.Encryption
{
    ...
    public sealed class EncryptionService : IEncryptionService, IDisposable
    {
        ...
        public EncryptionService(ILogger<IEncryptionService> logger, ISecureStorage storage, string masterKeyId = "NucleusAF-masterkey")
        {
        ...
        public async ValueTask<IEncryptionContext> GetEncryptionContextAsync(Type configType, Guid id)
        {
            ...
            this.logger.LogDebug("[EncryptionService] Created encryption context for {ConfigType} with id {Id}", configType.Name, id);
        ...
        public byte[] Encrypt(byte[] plaintext, byte[] key)
        {
            ...
            this.logger.LogDebug("[EncryptionService] Encrypted {PlaintextLength} bytes to {ResultLength} bytes", plaintext.Length, result.Length);
        ...
        public byte[] Decrypt(byte[] blob, byte[] key)
        {
            ...
            if (cryptoMarker == CryptoSchema.MarkerAesGcmV1)
            {
                ...
                this.logger.LogDebug("[EncryptionService] Decrypted {BlobLength} bytes to {ResultLength} bytes", blob.Length, result.Length);

                return result;
            }
            else
            {
                this.logger.LogError("[EncryptionService] Unknown crypto marker encountered: 0x{CryptoMarker:X2}", cryptoMarker);
        ...
        private async Task<byte[]> LoadOrCreateMasterKeyAsync()
        {
            ...
            if (keyIsValidSize)
            {
                this.logger.LogDebug("[EncryptionService] Loaded existing master key");
        ...
        private async Task<ReadOnlyMemory<byte>?> TryLoadExistingMasterKeyAsync()
        {
            ...
            {
                this.logger.LogWarning("[EncryptionService] Failed to load existing master key: {Error}", ex.Message);
        ...
        private async Task<byte[]> CreateNewMasterKeyWithLocking()
        {
            ...
            try
            {
                ...
                {
                    this.logger.LogDebug("[EncryptionService] Master key was created by another thread during lock wait");
        ...
        private async Task<byte[]> GenerateAndSaveNewMasterKey()
        {
            ...
            try
            {
                ...
                this.logger.LogInformation("[EncryptionService] Generated and saved new master key");

                return newKey;
            }
            catch (Exception ex)
            {
                this.logger.LogError("[EncryptionService] Failed to save new master key: {Error}", ex.Message);
        ...
        private async Task<byte[]> LoadOrCreateContextKeyAsync(string contextBlobId, byte[] masterKey)
        {
            ...
            return encryptedContextKey != null
                ? this.Decrypt(encryptedContextKey, masterKey)
                : await this.CreateNewContextKeyAsync(contextBlobId, masterKey).ConfigureAwait(false);
        }
        private async Task<byte[]?> LoadExistingContextKeyAsync(string contextBlobId)
        {
            try
            {
                ...
                {
                    this.logger.LogDebug("[EncryptionService] Loaded existing context key for blob id: {BlobId}", contextBlobId);
            ...
            {
                this.logger.LogWarning("[EncryptionService] Failed to load context key for blob id {BlobId}: {Error}", contextBlobId, ex.Message);
        ...
        private async Task<byte[]> CreateNewContextKeyAsync(string contextBlobId, byte[] masterKey)
        {
            ...
            try
            {
                ...
                this.logger.LogDebug("[EncryptionService] Created and saved new context key for blob id: {BlobId}", contextBlobId);

                return newContextKey;
            }
            catch (Exception ex)
            {
                this.logger.LogError("[EncryptionService] Failed to save new context key for blob id {BlobId}: {Error}", contextBlobId, ex.Message);
        ...
        private void ValidateEncryptionKey(byte[] key)
        {
            ...
            if (!keyIsCorrectSize)
            {
                this.logger.LogError("[EncryptionService] Invalid encryption key size: {ActualSize}, expected: {ExpectedSize}", key.Length, KeySizeBytes);
        ...
        private void ValidateDecryptionInputs(byte[] blob, byte[] key)
        {
            ...
            {
                this.logger.LogError("[EncryptionService] Blob too small for decryption: {ActualSize}, minimum required: {MinimumSize}", blob.Length, 1 + NonceSize + TagSize);
            ...
            if (!keyIsCorrectSize)
            {
                this.logger.LogError("[EncryptionService] Invalid decryption key size: {ActualSize}, expected: {ExpectedSize}", key.Length, KeySizeBytes);
