...
namespace NucleusAF.Extensions
{
    public static class TypeExtensions
    {
        ...
        public static T? GetStaticPropertyValue<T>(this Type type, string propertyName)
        {
            ...
            return memberFoundAfterRefresh && member != null
                ? GetValueFromMember<T>(propertyName, member)
                : throw new ArgumentException($"The property or field '{propertyName}' does not exist on type '{type.FullName}' or its ancestors.", nameof(propertyName));
        }

        /// <summary>
        /// Extracts static public fields and properties from <paramref name="sourceType"/> and returns them
        /// as a dictionary of parameter name to value. This is useful for module metadata extraction
