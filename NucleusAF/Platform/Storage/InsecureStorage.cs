...
namespace NucleusAF.Platform.Storage
{
    public sealed class InsecureStorage : ISecureStorage
    {
        ...
        public void Initialize(string newRootDirectory, bool moveExisting = true, bool overwrite = false)
        {
            ...
            this.logger.LogInformation("[InsecureStorage] Storage initialized with directory: {Directory}", newRootDirectory);
        }
        ...
        public async ValueTask<ReadOnlyMemory<byte>?> TryLoadAsync(string id, CancellationToken ct = default)
        {
            ...
            if (!fileExists)
            {
                this.logger.LogDebug("[InsecureStorage] SecureStore: [{Id}] not found – returning null", id);
        ...
        private async Task<ReadOnlyMemory<byte>> LoadFileWithLocking(string id, string filePath, CancellationToken ct)
        {
            ...
            try
            {
                ...
                this.logger.LogDebug("[InsecureStorage] SecureStore: [{Id}] loaded {Count} bytes", id, fileData.Length);
        ...
        private async Task SaveFileWithLocking(string id, string filePath, ReadOnlyMemory<byte> data, CancellationToken ct)
        {
            ...
            try
            {
                ...
                this.logger.LogDebug("[InsecureStorage] SecureStore: [{Id}] saving {Length} bytes (update = {Updating})", id, data.Length, isUpdate);
                ...
                this.logger.LogInformation("[InsecureStorage] SecureStore: [{Id}] {Action}", id, operationType);
        ...
        private async Task DeleteFileWithLocking(string id, string filePath, CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (!fileExists)
                {
                    this.logger.LogDebug("[InsecureStorage] [{Id}] delete requested but file missing", id);
                ...
                this.logger.LogInformation("[InsecureStorage] [{Id}] deleted", id);
        ...
        private void MigrateExistingFiles(string newRootDirectory, bool overwrite)
        {
            ...
            foreach (string sourceFile in existingFiles)
            {
                ...
                if (migrationSuccessful)
                    this.logger.LogDebug("[InsecureStorage] Migrated file: {SourceFile}", sourceFile);
            }

            this.logger.LogInformation("[InsecureStorage] Completed file migration from {OldDirectory} to {NewDirectory}", this.storeRootDirectory, newRootDirectory);
        }
        ...
        private bool TryMigrateFile(string sourceFile, string newRootDirectory, bool overwrite)
        {
            ...
            {
                this.logger.LogWarning("[InsecureStorage] Failed to migrate file {SourceFile}: {Error}", sourceFile, ex.Message);
        ...
        private async Task<IEnumerable<string>> DiscoverStorageFiles(string? prefix, CancellationToken ct)
        {
            ...
            try
            {
                this.logger.LogTrace("[InsecureStorage] SecureStore: listing ids (prefix = '{Prefix}')", prefix);
        ...
        {
            return prefix == null ? true : id.StartsWith(prefix, StringComparison.Ordinal);
        }
        ...
        private void PerformDisposal()
        {
            ...
            try
            {
                ...
                this.logger.LogDebug("[InsecureStorage] InsecureStorage disposed");
            }
            finally
            {
                this.systemLock.Release();
